...
export const command = async (
  ...
  if (!field.length || (!content && mention)) {
    let roles;
    if (mention) {
      const mentionMember = message.guild
        ? message.guild.member(mention)
        : null;
      if (mentionMember) {
        ...
        .set(
          {
            ...
            joinedAt: firebase.firestore.Timestamp.fromDate(
