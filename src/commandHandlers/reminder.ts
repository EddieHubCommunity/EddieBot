  ...
  try {
    ...
    if (isNaN(dateTimestamp)) {
      return buildErrorMessage(
        'The provided date format on the second parameter is not valid, please use the ISO format.'
      );
    }
  ...
  async function reminderCallback() {
    try {
      await message.author.send(
        `${CLOCK_EMOJI} Hey, just to remind you: **${reminderMessage}**`
      );
    } catch (e) {
      log.error(
        `ERROR: Sending the reminder message to the user with id: ${message.author.id}`,
        e.message
      );
      log.error(e);
    }
